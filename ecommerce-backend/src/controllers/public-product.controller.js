...
import ProductService from "../services/publicProduct.service.js";
...
class publicProductController {
  ...
  countProduct = async (req, res) => {
    const { category_id, sub_cat_id } = req.body;
    try {
      const _condition = {
        is_active: true,
        // created_by: {
      ...
      var product = await this.productService.productCounts(_condition);

      response(res, HTTP_STATUS.SUCCESS, "product_get", product);
      return;
    } catch (error) {
      response(res, HTTP_STATUS.INTERNAL_SERVER_ERROR, "internal_server_error");
      return;
