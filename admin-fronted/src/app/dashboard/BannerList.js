...
const BannerList = () => {
  ...
  const FETCH_ALL_BANNERS = `${BASE_URL}banner/`;
  ...
  const { TblContainer, TblHead, TblPagination, recordsAfterPagingAndSorting } =
    useTable(newBanner, headCells, filterFn);
  ...
  const onDelete = async (id) => {
    ...
    try {
      const deleteresponse = await axios.delete(`${BASE_URL}banner/${id}`, {
        headers: { Authorization: `${TOKEN}` },
      });
      const { data } = await axios.get(FETCH_ALL_BANNERS, {
        headers: { Authorization: `${TOKEN}` },
              ...
              {loading ? (
                recordsAfterPagingAndSorting().map((item, index) => (
                  <TableRow key={item._id}>
                    ...
                    {/* <TableCell>
                                            {
                                                item.image == "" ? <img src="assets/images/noimage.jpg" alt="noimage"/> : <img width={100} src={item?.image} />
                                            }                          
                                        </TableCell> */}
                    <TableCell>
                      { loading ?
                        <img
                          style={mystyle}
                          src={`https://inani-hub.s3.amazonaws.com/${item?.image}`}
                        /> :<div>{""}</div>
                      }
                    </TableCell>
                    <TableCell>
